...
// biome-ignore lint/security/noSecrets: TypeScript source text, not a credential.
const EMPTY_SET = 'new Set<string>()';
const setLiteral = (names) =>
  names.length > 0 ? `new Set<string>([\n${names.map((n) => `  '${n}',`).join('\n')}\n])` : EMPTY_SET;
...
export const serverRenderedPages = ${setLiteral(serverRendered)};
...
export const ssrClientOnly = ${setLiteral(clientOnly)};
